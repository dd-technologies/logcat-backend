...
const getAllEvents = async (req, res) => {
  try {
    ...
    const allEvents = await event_ventilator_collection
    .find().sort("date: 1");
...
const createTrends = async (req, res, next) => {
  try {
    ...
    const modelReference = require(`../model/${collectionName}`);
    console.log(modelReference,'modelReference');
    const { time,did, type,mode,pip,peep,mean_Airway,vti,vte,mve,mvi,fio2,respiratory_Rate,ie,tinsp,texp,averageLeak} = req.body;
    if (!time || !did || !type ||!mode|| !pip|| !peep|| !mean_Airway|| !vti|| !vte|| !mve|| !mvi|| !fio2|| !respiratory_Rate|| !ie|| !tinsp|| !texp|| !averageLeak) {
      return res.status(400).json({
        status: 0,
        data: {
          err: {
